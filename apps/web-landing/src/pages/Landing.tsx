...
import { api, PORTAL_URLS } from "@/api/client";
import { cn } from "@/lib/utils";

type Role = "shipper" | "carrier" | "broker";
...
export function Landing() {
  ...
  function redirectToPortal(role: Role) {
    window.location.href = PORTAL_URLS[role];
  }

  async function handleSubmit() {
    if (!selectedRole) return;
    if (!api) {
      setError("Registration and sign-in are temporarily unavailable while service activation is completed.");
      return;
    }
    await run(async () => {
      const client = api!;
  ...
  return (
    <div className="relative flex min-h-screen flex-col" style={{ backgroundColor: NAVY }}>
      {/* Dot-grid texture */}
      <div
        aria-hidden
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 h-[480px] w-[900px] rounded-full opacity-[0.08]"
          ...
          <div className="w-full max-w-md animate-fade-in">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-8 shadow-2xl backdrop-blur-sm">
              ...
              <div className="space-y-3">
                {authMode === "signup" && (
                  <>
                    <DarkField label="Full name" value={form.name} onChange={(v) => setField("name", v)} placeholder="Your full name" autoComplete="name" />
                    <DarkField label="Company name" value={form.tenantName} onChange={(v) => setField("tenantName", v)} placeholder="Company Ltd." autoComplete="organization" />
                    <DarkField label="VAT / TIN" value={form.vatNumber} onChange={(v) => setField("vatNumber", v.replace(/\D/g, ""))} placeholder="9-digit number" inputMode="numeric" maxLength={9} />
                    <div className="grid grid-cols-2 gap-3">
                      <DarkField label="Country" value={form.country} onChange={(v) => setField("country", v)} />
                      <DarkField label="City" value={form.city} onChange={(v) => setField("city", v)} />
                    </div>
                    <DarkField label="Address" value={form.address} onChange={(v) => setField("address", v)} placeholder="Registered address" autoComplete="street-address" />
                    <DarkField label="Phone" type="tel" value={form.phone} onChange={(v) => setField("phone", v)} placeholder="+998 XX XXX XX XX" autoComplete="tel" />
                  </>
                )}

                <DarkField label="Email" type="email" value={form.email} onChange={(v) => setField("email", v)} placeholder="you@company.com" autoComplete="email" />
                <DarkField
                  label="Password"
                  type="password"
                  value={form.password}
                ...
                <button
                  ...
                  disabled={busy}
                  className="mt-1 flex w-full items-center justify-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-slate-900 shadow transition-all hover:bg-slate-100 active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50"
                >
...
function BrandMark() {
  return (
    <span className="inline-flex items-center gap-2.5">
      <svg width={40} height={40} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="EPL Move">
      ...
      <span className="leading-none">
        <span className="block text-xl font-extrabold tracking-tight text-white">
          EPL <span style={{ color: GOLD }}>M</span>OVE
        </span>
