  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            placeholder="Search by email or tenant…" value={search} onChange={(e) => setSearch(e.target.value)}
          />
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Users",   value: MEMBERS.length,                                  tone: "slate" },
          { label: "Shipper Users", value: MEMBERS.filter((m) => m.roleKey.startsWith("shipper")).length, tone: "blue"  },
          { label: "Carrier Users", value: MEMBERS.filter((m) => m.roleKey.startsWith("carrier")).length, tone: "teal"  },
        ].map((s) => (
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        <div className="border-b border-slate-100 px-5 py-3 flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-slate-400">{visible.length} users</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                {["Email", "Tenant", "Role", "User ID", "Joined", ""].map((h) => (
