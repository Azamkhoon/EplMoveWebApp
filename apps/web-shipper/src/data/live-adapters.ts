...
import type { Load as LiveLoad, Shipment as LiveShipment, Quote as LiveQuote } from "@epl/sdk";
...
export function toViewShipment(s: LiveShipment, load?: LiveLoad): ViewShipment {
  ...
  return {
    ...
    createdAt: load?.createdAt ?? s.bookedAt,
    pickupDate: load?.readyDate ?? s.bookedAt,
    etaDate: s.etaDate ?? s.bookedAt,
    weightKg: load?.weightKg ?? 0,
    volumeM3: load?.volumeM3 ?? 0,
    pieces: load?.pieces ?? 0,
    valueUsd: load?.value?.amount ?? value,
    costUsd: value,
    progress: s.progress ?? 0,
    currentLocation: undefined,
    documents: [],
    events: s.activity?.length
      ? s.activity.map((event) => ({
          id: event.id,
          status: event.title,
          description: event.description ?? event.type.replace(/\./g, " "),
          location: "EPL Move",
